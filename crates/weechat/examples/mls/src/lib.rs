// Copyright (C) 2023 Ryan Lahfa <ryan@lahfa.xyz>
//
...
//! Experimental MLS support in Weechat
...
impl Plugin for MLS {
    fn init(_: &Weechat, _args: Args) -> Result<Self, ()> {
        ...
        let command_settings = CommandSettings::new("mls")
            .description("Manipulate internal MLS state for current groups")
            .add_argument("[name]")
...
plugin!(
    MLS,
    name: "mls",
    author: "Ryan Lahfa <ryan@lahfa.xyz>",
    description: "Experimental MLS support for Weechat",
    version: "0.1.0",
    license: "GPL3"
);
